 ...
 * it does NOT lighten for dark mode; `accentText` is the value for teal as text
 * on the canvas. Collapsing the two would silently fail one role or the other.
 */
describe('accent stays a fill; accentText carries the text role', () => {
  ...
  // an accentSoft chip. Left at its light value it lands at 2.07:1 on the dark
  // chip. This is the regression the palette pair exists to prevent, so assert
  // the inversion rather than trusting the comment.
...
/**
 * The onInk family labels fills whose lightness inverts between themes — `ink`
 * (active-delivery card, Navigate button), `green` (Accept) and `ink3` (its
 * expired state). A fixed white label scored 2.27:1 on the dark green button.
 */
describe('onInk family inverts so labels survive on inverting fills', () => {
  ...
  it('flips the muted and faint steps too', () => {
    expect(darkColors.onInkMuted).not.toBe(lightColors.onInkMuted);
    expect(darkColors.onInkFaint).not.toBe(lightColors.onInkFaint);
  });

  // The three steps must stay distinct in both themes, or the hierarchy the
  // active-delivery card relies on collapses into one flat colour.
  it('keeps the three steps distinct within each theme', () => {
    for (const p of [lightColors, darkColors]) {
      expect(new Set([p.onInk, p.onInkMuted, p.onInkFaint]).size).toBe(3);
    }
  });
});
