...
export function usePulse(active: boolean, peak = 1.04) {
  ...
  useEffect(() => {
    if (!active) {
      ...
      // unmounts during this exact 160ms (a ticket is accepted, an offer is
      // dismissed — the same event that de-escalated it). Returning no cleanup
      // on this path would leave that animation running on a dead component.
