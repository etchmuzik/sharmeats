...
/**
 * Revoke the current device's push identity through either authoritative path:
 * delete the backend mapping or invalidate the native Expo registration.
 */
export async function revokePushIdentity(
  ...
  if (revoked) {
    await deps.clearStoredToken().catch(() => undefined);
  }
  return revoked;
}
