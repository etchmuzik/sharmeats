/**
 * Runtime theming for the driver app.
 *
 * WHY THIS EXISTS AT ALL: most of this app styles inline, but the files that do
 * use `StyleSheet.create({ color: colors.ink })` evaluate it ONCE, when the
 * module is first imported. The literal is copied into the style object and
...
const THEME_MODE_STORAGE_KEY = '@sharmeats/driver/theme-mode';
...
interface ThemeValue {
  ...
  scheme: ColorScheme;
  /** What the driver picked, which may be `system`. */
  mode: ThemeMode;
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  // Starts at 'system' rather than blocking the tree on a disk read. A driver
  // who has never touched the control sees the correct theme immediately; only
  // someone who picked an explicit override can catch a brief first frame in
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(THEME_MODE_STORAGE_KEY)
      ...
      // A preference we cannot read is not worth surfacing mid-shift — the
      // driver keeps the system theme and can re-pick from the header.
      .catch(() => {});
    return () => {
      cancelled = true;
    };
...
/** The picked mode plus its setters, for the header control. */
