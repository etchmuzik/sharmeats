...
export default function OrderTracking() {
  ...
  const t = useT();
  // This screen is the one customers stare at for half an hour, and it had NO
  // direction handling at all: nineteen row layouts, every one of them pinned
  // LTR, so the entire tracking experience rendered half-mirrored in Arabic —
  // Arabic text right-aligned inside rows whose icons, bullets and prices were
  // still on the left. `useDirection` mirrors instantly with the in-app language
  // switcher (see the module comment for why not I18nManager.forceRTL).
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <View style={styles.map}>
        ...
        {driverLoc && (
          <View style={[styles.liveBadge, dir.row, dir.isRtl ? { right: 14 } : { left: 14 }]}>
      ...
      <ScrollView
        ...
        <View style={[styles.etaRow, dir.row]}>
          <View style={{ flex: 1 }}>
            {isCancelled ? (
              <>
                ...
                <Text style={[styles.etaBig, dir.text, { color: colors.red }]}>
          ...
          {/* The trailing column hugs the far edge, which mirrors with the row. */}
              ...
              <View key={s.key} style={[styles.step, dir.row]}>
                {i > 0 && (
                  <View
                    style={[
                      styles.connector,
                      // The connector is absolutely positioned against the
                      // bullet column, so it has to swap sides with the row.
                      dir.isRtl ? { right: 10 } : { left: 10 },
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {histEntry && <Text style={[styles.stTime, dir.text]}>{formatTime(new Date(histEntry.at))}{histEntry.note ? ` · ${histEntry.note}` : ''}</Text>}
                </View>
              </View>
            );
          })}
        ...
        {((order.aggregateAllergens && order.aggregateAllergens.length > 0) || order.kitchenNotes) && (
          <View style={styles.briefingCard}>
            ...
            {order.kitchenNotes && (
              <Text style={[styles.briefingNotes, dir.text]}>{order.kitchenNotes}</Text>
            )}
          </View>
        )}
        ...
        <View style={styles.summary}>
          ...
          {order.items.map((it) => (
            <View key={it.lineId} style={[styles.summaryLine, dir.row]}>
              <Text style={[styles.summaryQ, dir.text]}>{it.quantity}×</Text>
              <Text style={[{ flex: 1, fontSize: font.sizes.lg, color: colors.ink }, dir.text]}>
                {it.name}
              </Text>
            </View>
          ))}
          ...
          <View style={styles.contactCard}>
            {restaurant.address && (
              <View style={[styles.contactAddressRow, dir.row]}>
                ...
                <Text style={[styles.contactAddr, dir.text]} numberOfLines={2}>
                  {restaurant.address}
                </Text>
              </View>
            )}
...
const useStyles = makeStyles((colors) => ({
  ...
  // No `left` here on purpose: the reading-edge side is supplied at the call
  // site so RTL can pin it to `right` instead. Setting both would stretch it.
  liveBadge: {
    position: 'absolute',
    bottom: 14,
    flexDirection: 'row',
  ...
  // Same as liveBadge: the horizontal edge comes from the call site.
  ...
  // Horizontal edge supplied at the call site so it mirrors in RTL.
