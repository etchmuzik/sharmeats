...
-- place_order's argument LIST is changing (2 new trailing params), not just its
-- body, so `create or replace function` below would ADD a second overload
-- rather than replacing the existing one (Postgres identifies functions by
-- name + full parameter signature). Left alone, that leaves both the old
-- 10-arg and new 12-arg place_order live simultaneously; any caller invoking
-- it with only the original 4 required named args (every existing caller)
-- then hits "function ... is not unique" because both overloads match via
-- trailing defaults. Verified locally (see task-1-report.md) — this DROP is
-- required for the "no existing caller breaks" claim to hold.
